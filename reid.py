...
class SiameseDatasetTrain(Dataset):
    ...
    def _get_random_positive(self, seq, frame_id, obj_id):
        ...
        elig_boxes = self.ground_truth[
            ...
            & (
                abs(self.ground_truth["frame_id"] - frame_id).between(
                    1, self.max_frame_gap
                )
            )
        ]
    ...
    def __getitem__(self, idx):
        ...
        if random.random() < self.pos_prob:
            ...
            img2 = self._get_random_positive(
                box1["seq"], box1["frame_id"], box1["obj_id"]
            )
            label = 1
        else:
            # Get negative example
            img2 = self._get_random_negative(
                box1["seq"], box1["frame_id"], box1["obj_id"]
            )
            label = 0
...
def create_reid_dataset(val_size=0.2, train_batch_size=64, val_batch_size=64):
    ...
    siamese_dataset = SiameseDatasetTrain(
        data_dir=data_dir_train, crop_size=256, pos_prob=0.5, max_frame_gap=10
    )

    train_dataset, val_dataset = random_split(siamese_dataset, val_size=val_size)

    train_siamese_dataloader = DataLoader(
        train_dataset,
    ...
    val_siamese_dataloader = DataLoader(
        val_dataset,
